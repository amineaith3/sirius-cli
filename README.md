...
- **Pluggable Frontend Engine (`--frontend`)**: Choose between **React** (TypeScript + Vite + Tailwind CSS), **Vue 3** (TypeScript + Vite + Tailwind CSS + Pinia + Vue Router), or **SvelteKit** (TypeScript + SvelteKit + Tailwind CSS + stores) via a single flag. All frontends share identical UI design, interactive SVG/Recharts trend charts, and CRUD capability sets.
