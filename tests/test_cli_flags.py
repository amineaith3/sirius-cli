...
def test_flag_csv_and_no_seed(tmp_project_dir, sample_csv_file):
    ...
    result = runner.invoke(app, ["init", project_path, "--csv", sample_csv_file, "--no-seed"])
...
def test_flag_db(tmp_project_dir, sample_sqlite_db):
    ...
    result = runner.invoke(app, ["init", project_path, "--db", sample_sqlite_db, "--no-seed"])
    assert result.exit_code == 0
...
def test_flag_pg(tmp_project_dir, sample_config_json):
    ...
    result = runner.invoke(app, ["init", project_path, "--config", sample_config_json, "--pg"])
    assert result.exit_code == 0
...
def test_flag_mysql(tmp_project_dir, sample_config_json):
    ...
    result = runner.invoke(app, ["init", project_path, "--config", sample_config_json, "--mysql"])
    assert result.exit_code == 0
...
def test_flag_auth(tmp_project_dir, sample_config_json):
    ...
    result = runner.invoke(app, ["init", project_path, "--config", sample_config_json, "--auth"])
    assert result.exit_code == 0
...
def test_flag_admin_creds(tmp_project_dir, sample_config_json):
    ...
    result = runner.invoke(app, ["init", project_path, "--config", sample_config_json, "--auth", "--admin-user", "superadmin", "--admin-pass", "secret123"])
    assert result.exit_code == 0
...
def test_flag_port(tmp_project_dir, sample_config_json):
    ...
    result = runner.invoke(app, ["init", project_path, "--config", sample_config_json, "--port", "9999"])
    assert result.exit_code == 0
...
def test_flag_api_url(tmp_project_dir, sample_config_json):
    ...
    result = runner.invoke(app, ["init", project_path, "--config", sample_config_json, "--api-url", "https://api.myprod.com"])
    assert result.exit_code == 0
...
def test_flag_theme(tmp_project_dir, sample_config_json):
    ...
    result = runner.invoke(app, ["init", project_path, "--config", sample_config_json, "--theme", "emerald"])
    assert result.exit_code == 0
...
def test_flag_conflicting_inputs(tmp_project_dir, sample_config_json, sample_csv_file):
    ...
    result = runner.invoke(app, ["init", project_path, "--config", sample_config_json, "--csv", sample_csv_file])
